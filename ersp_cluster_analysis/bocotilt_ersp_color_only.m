...
PATH_TF_DATA     = '/mnt/data_dump/bocotilt/4_ersp_color_only/';
PATH_FIELDTRIP   = '/home/plkn/fieldtrip-master/';
PATH_OUT         = '/mnt/data_dump/bocotilt/4_ersp_color_only/out/';
...
if ismember('part1', to_execute)
    ...
    for s = 1 : length(subject_list)
        ...
        % Get condition idx
        idx_std_rep = EEG.trialinfo(:, 4) == 0 & EEG.trialinfo(:, 10) == 0 & EEG.trialinfo(:, 5) == 0;
        idx_std_swi = EEG.trialinfo(:, 4) == 0 & EEG.trialinfo(:, 10) == 1 & EEG.trialinfo(:, 5) == 0;
        idx_bon_rep = EEG.trialinfo(:, 4) == 1 & EEG.trialinfo(:, 10) == 0 & EEG.trialinfo(:, 5) == 0;
        idx_bon_swi = EEG.trialinfo(:, 4) == 1 & EEG.trialinfo(:, 10) == 1 & EEG.trialinfo(:, 5) == 0;
...
if ismember('part3', to_execute)
    ...
    % Cluster 5 stuff ========================================================================================================================================
    idx_time = logical(squeeze(mean(clusts(5).idx, [1, 2])));
    idx_freq = logical(squeeze(mean(clusts(5).idx, [1, 3])));
    idx_chan = logical(squeeze(mean(clusts(5).idx, [2, 3])));

    idx_ersp = [19, 99, 100];

    outline_cluster_5 = logical(squeeze(mean(clusts(5).idx, 1)));
    outline_cluster_5_ersp = logical(squeeze(mean(clusts(5).idx(idx_ersp, :, :), 1)));
    apes_cluster_5 = squeeze(mean(adjpetasq_switch(idx_chan, :, :), 1));
    ersp_rep_cluster_5 = squeeze(mean(double((ersp_std_rep(:, idx_ersp, :, :) + ersp_bon_rep(:, idx_ersp, :, :)) / 2), [1, 2]));
    ersp_swi_cluster_5 = squeeze(mean(double((ersp_std_swi(:, idx_ersp, :, :) + ersp_bon_swi(:, idx_ersp, :, :)) / 2), [1, 2]));

    writematrix(outline_cluster_5, [PATH_OUT, 'outline_cluster_5.csv']);
    writematrix(outline_cluster_5_ersp, [PATH_OUT, 'outline_cluster_5_ersp.csv']);
    writematrix(apes_cluster_5, [PATH_OUT, 'apes_cluster_5.csv']);
    writematrix(ersp_rep_cluster_5, [PATH_OUT, 'ersp_rep_cluster_5.csv']);
    writematrix(ersp_swi_cluster_5, [PATH_OUT, 'ersp_swi_cluster_5.csv']);
    ...
    colormap('jet')
    set(gca, 'clim', [-0.2, 0.2])
    saveas(gcf, [PATH_OUT, 'topo_cluster_5.png']);
