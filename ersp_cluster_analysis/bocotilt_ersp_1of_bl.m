...
PATH_TF_DATA     = '/mnt/data_dump/bocotilt/4_ersp_1of_bl/';
PATH_FIELDTRIP   = '/home/plkn/fieldtrip-master/';
PATH_OUT         = '/mnt/data_dump/bocotilt/4_ersp_1of_bl/out/';
...
if ismember('part1', to_execute)
    ...
    for s = 1 : length(subject_list)
        ...
        parfor ch = 1 : EEG.nbchan
            ...
            % Calculate ersp
            ersp_std_rep(s, ch, :, :) = single(squeeze(mean(powcube(:, :, idx_std_rep), 3)));
            ersp_std_swi(s, ch, :, :) = single(squeeze(mean(powcube(:, :, idx_std_swi), 3)));
            ersp_bon_rep(s, ch, :, :) = single(squeeze(mean(powcube(:, :, idx_bon_rep), 3)));
            ersp_bon_swi(s, ch, :, :) = single(squeeze(mean(powcube(:, :, idx_bon_swi), 3)));
