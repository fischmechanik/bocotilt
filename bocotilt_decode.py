...
def decode_timeslice(X, trialinfo):

    # Specify bootstrapping
    n_batch = 3
    n_bootstrap_train = 400
    n_bootstrap_test = 100

    # Result matrices
    acc_true = np.zeros((5, 2, 2))
    acc_fake = np.zeros((5, 2, 2))
    fmp_true = np.zeros((5, 2, 2, X.shape[1]))
    fmp_fake = np.zeros((5, 2, 2, X.shape[1]))
...
for dataset_idx, dataset in enumerate(datasets):
    ...
    if os.path.isfile(dataset):
        print(f"Dataset {dataset_idx + 1} / {len(datasets)} already decoded.")
        continue

    # Try decoding dataset
    try:
        ...
        # Get subject
        id_string = dataset.split("auto")[0].split("/")[-1]
        ...
        # Load trialinfo (cols: id, age, relevant, colpat, fisec, stim, interruption, probert, probeacc)
        trialinfo = np.genfromtxt(
            dataset.split("auto")[0] + "trialinfo.csv", delimiter=","
        )
    ...
    # In case of an error
        ...
        list_failed.append(id_string)
        print(f"decoding of dataset {id_string} failed")
